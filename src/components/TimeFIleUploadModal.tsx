...
import { TimedGroupContent, UserModel } from '../models/AllModel';
import uploadImage from '../hooks/fileUpload';
import { serverString } from '../models/ServerString';
import axios from 'axios';

interface TimeFIleUploadModalProps{
    isOpen: boolean;
    onClose: () => void;
    groupId:string;    
}

const TimeFIleUploadModal:React.FC<TimeFIleUploadModalProps> = ({isOpen,onClose,groupId}) => {
const user = useSelector(GetUser) as UserModel;
const [loading,setLoading] = useState<boolean>(false);
const [selectedFile,setSelectedFile] = useState<File | null>(null);
const [fileData,setFileData] = useState<TimedGroupContent>({  
  title: '',
  sentBy: '',
  fileAddress: '',
  otherData:'',
...
const [time, setTime] = useState<string>('');
    ...
    const handleSubmit = async(e: React.FormEvent<HTMLFormElement>)=>{
        ...
        const url = `${serverString}/api/user/createFileUploadTime?groupId=${groupId}`;

        const x = `${date}-${time}`
      
        const response = await axios.post(url,{...fileData,sentBy:user._id,fileAddress:uploadedFile,dateToBeSentOn:x});
...
export default TimeFIleUploadModal
