...
const DeleteAdminModal:React.FC<DeleteAdminModalProps> = ({isOpen,onClose}) => {
    const [loading,setLoading] = useState<boolean>(false);
    const [groupData,setGroupData] = useState<UserModel[] | null>(null);
    ...
    const getAdminData = async() =>{
        setLoading(true);
        const url = `${serverString}/api/user/getAdmins`;
    ...
    const handleDelete = async(id:string)=>{
        setLoading(true);
        const url = `${serverString}/api/user/removeAdmin?id=${id}`;
        const response = await axios.get(url);
        if(response.status === 200){
            setLoading(false);
        onClose();
                                        ...
                                        groupData?.map((group) => (
                                            <div className='w-full rounded-md bg-slate-100 dark:bg-slate-700 flex flex-row items-center justify-between p-3 my-2 hover:cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-500' key={group._id}>
                                                <div className='flex flex-col justify-start items-center'>
                                                    ...
                                                    <h2 className='ml-2 dark:text-white'>{group.email}</h2>
                                                    <h2 className='ml-2 dark:text-white'>{group.phoneNo}</h2>
                                                </div>
                                                <div>
                                                    {(user._id === group._id ) ? (
                                                        <button className='py-1 px-4 bg-gray-500 rounded-md cursor-not-allowed' disabled>You</button>
                                                    ) : (
                                                        <button className='py-1 px-4 bg-red-500 rounded-md ' onClick={()=>{handleDelete(group._id!)}} >Delete</button>
                                                    )}
                                                </div>
                                            </div>
                                        ))
