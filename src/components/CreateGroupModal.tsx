...
const [addGroupData, setAddGroupData] = useState<Group>({
    groupAdminID: '',
    groupName: '',
    groupProfile:'',
    groupMembers: [],
...
const handleSubmit = async(e:React.FormEvent<HTMLFormElement>) =>{
    setLoading(true);
    e.preventDefault();
    const uploadedImage = await uploadImage(imagePreview!,addGroupData.groupName,'groupAvtar')

    const url = `${serverString}/api/user/createGroup`;
    const response = await axios.post(url,{...addGroupData,groupAdminID:user._id,groupProfile:uploadedImage})
                                        ...
                                        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
...

                                            {
                                                imagePreview?(
                                                    ...
                                                    <h2 className='text-center'>select to show preview </h2>
                                                )
                                            }
